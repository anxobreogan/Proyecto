...
async function login(req, res, next) {
  ...
  try {
    ...
    return res.satatus(400).send(e);
  }

  try {
    const connection = await mysqlPool.getConnection();
    const sqlQuery = `SELECT idusuarios,uuid,email,password,activatedat FROM users WHERE email= '${accountData.email}'`;
...
async function login(req, res, next) {
  ...
  try {
    ...
    idusuarios, uuid, email, password, activatedat
    FROM users
